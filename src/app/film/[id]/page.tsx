...
// Вспомогательная функция для форматирования длительности (например, "2 ч. 5 м.")
...
interface FilmPageProps {
  params: {
    id: string; // ID фильма из динамического маршрута [id]
  };
}

// Главный компонент страницы фильма
export default function FilmPage({ params }: FilmPageProps) {
  // Правильно извлекаем id из params для клиентского компонента
  const { id } = params;
      ...
      const fetchFilmAndProgress = async () => {
        ...
        try {
          ...
          if (user?.uid) {
            ...
            // Если пользователь не авторизован, сбрасываем закладку
            setIsBookmarked(false);
          }
        } catch (error) {
  ...
  // Обработчик для переключения статуса закладки
  ...
  return (
    <main className="container mx-auto p-4 pt-8 min-h-screen bg-gray-950 text-white">
      <div className="bg-gray-800 p-8 rounded-lg shadow-xl">
        ...
        {film.originalTitle && (
          <h2 className="text-xl text-gray-300 mb-4">{film.originalTitle} ({film.year})</h2>
        )}
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="md:w-2/3 space-y-4 text-lg">
            ...
            {film.dubbingStudio && (
              <p>
                <span className="font-semibold text-gray-400">Студия дубляжа:</span> {film.dubbingStudio}
              </p>
            )}
            ...
            <div className="mt-8">
              ...
              <div className="player-wrapper relative pt-[56.25%] bg-gray-700 rounded-lg overflow-hidden">
                {film.videoUrl ? (
                  <iframe
                    src={film.videoUrl}
                    className="absolute top-0 left-0 w-full h-full rounded-xl border-0 shadow-2xl"
                    allowFullScreen
                    title={film.title}
                  ></iframe>
                ) : (
